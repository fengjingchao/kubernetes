...
type Master struct {
	*genericapiserver.GenericAPIServer
	...
	thirdPartyStorageConfig generic.StorageConfig
...
func (m *Master) InstallAPIs(c *Config) {
	...
	if c.APIResourceConfigSource.ResourceEnabled(extensionsapiv1beta1.SchemeGroupVersion.WithResource("thirdpartyresources")) {
		var err error
		config, err := c.StorageFactory.New(extensions.Resource("thirdpartyresources"))
		if err != nil {
			glog.Fatalf("Error getting third party storage: %v", err)
		}
		m.thirdPartyStorageConfig = *config
...
func (m *Master) initV1ResourcesStorage(c *Config) {
	...
	config, err := c.StorageFactory.New(api.Resource("services"))
	if err != nil {
		glog.Fatal(err.Error())
	}
	serviceStorageConfig := *config
...
func (m *Master) GetRESTOptionsOrDie(c *Config, resource unversioned.GroupResource) generic.RESTOptions {
	config, err := c.StorageFactory.New(resource)
	if err != nil {
